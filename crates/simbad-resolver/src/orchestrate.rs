...
use crate::{config::ResolverConfig, error::Error};
...
pub(crate) async fn resolve_core<R: Resolver, C: Cache>(
    ...
    let (simbad_query, caldwell_alias) =
        match simbad_resolver_caldwell::parse_caldwell_number(query) {
            Some(n) => match simbad_resolver_caldwell::caldwell_to_designation(n) {
