//! Offline HTTP-level tests for `SimbadSesameResolver` against a `wiremock`
//! mock of the CDS Sesame `-oxp` endpoint — no network, so these run in CI
...
async fn resolve_slow_endpoint_maps_to_timeout() {
    ...
    let resolver =
        SimbadSesameResolver::with_config(config_at(&server, Duration::from_millis(150)));
    let err = resolver.resolve("M 31").await.unwrap_err();
