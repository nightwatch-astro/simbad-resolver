//! [`SimbadSesameResolver`]: the CDS Sesame-backed [`Resolver`] impl.
...
use crate::config::default_sesame_config;
use crate::parse::{self, SesameHit};

/// Defensive cap on the Sesame response body (~8 MiB) — bounds memory/parse
/// time against a pathological or misbehaving endpoint; a real Sesame
/// `-oxp` reply for one target is a few KiB.
...
/// `SimbadTapResolver` — without this crate depending on `-tap` directly.
pub struct SimbadSesameResolver {
    config: SimbadConfig,
    client: reqwest::Client,
    enricher: Option<Arc<dyn Resolver>>,
}

impl SimbadSesameResolver {
    ...
    ///
    /// After a successful Sesame hit, [`Resolver::resolve`] resolves the
    /// coarse hit's primary designation through `enricher`; a successful
    /// enrichment replaces the coarse identity (object type + full alias
    /// set), a failed one falls back to the coarse Sesame identity. This
    /// crate has no build dependency on `-tap`: `enricher` is any trait
...
impl Default for SimbadSesameResolver {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Resolver for SimbadSesameResolver {
...
/// `NAME …` identifier, so a caller wanting a common name needs enrichment
/// (or a dedicated lookup) rather than a guess from the alias list.
...
/// Resolve a coarse Sesame identity through the optional enricher, falling
/// back to the coarse identity when unset or on enrichment failure.
///
/// Kept network-free (the caller passes an already-fetched `coarse`
/// identity) so the enrichment decision logic is unit-testable with a
/// `FakeResolver`, with no HTTP mocking required.
...
mod tests {
    use super::*;
    use simbad_resolver_core::{FakeResolver, ObjectType};
    ...
    fn with_config_uses_custom_endpoint() {
        ...
        let resolver = SimbadSesameResolver::with_config(config);
