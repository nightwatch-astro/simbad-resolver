//! Offline HTTP-level tests for `SimbadTapResolver`, exercising the full
...
use simbad_resolver_core::{ObjectType, PositionResolver, ResolveError, Resolver, SimbadConfig};
...
fn resolver_at(server: &MockServer, timeout: Duration) -> SimbadTapResolver {
    let config = SimbadConfig {
        endpoint: format!("{}/sim-tap/sync", server.uri()),
        timeout,
        user_agent: UA.to_owned(),
    };
    SimbadTapResolver::new(&config).expect("resolver builds from mock endpoint")
}

fn resolver(server: &MockServer) -> SimbadTapResolver {
